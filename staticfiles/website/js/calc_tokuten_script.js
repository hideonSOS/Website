...
function sortChart(ascending = true) {
  const combined = myChart.data.labels.map((label, i) => ({
    label,
    value: myChart.data.datasets[0].data[i],
  }));
  ...
  if (thrIdx !== -1) {
    ...
    myChart.data.datasets[thrIdx].data = Array(myChart.data.labels.length).fill(
      val
    );
  }
  myChart.update();
...
document
  ...
  .addEventListener("click", () => sortChart(true));
document
  .getElementById("sort-desc")
  .addEventListener("click", () => sortChart(false));

const thrInput = document.getElementById("threshold-input");
thrInput.addEventListener("input", (e) =>
  setOrUpdateThresholdLine(e.target.value)
);
