...
function readLocalRace() {
  try {
    return JSON.parse(localStorage.getItem(LS_RACE_KEY) || "[]");
  } catch {
    return [];
  }
}
function writeLocalRace(list) {
  localStorage.setItem(LS_RACE_KEY, JSON.stringify(list));
}
...
async function upsertRace(payload) {
  // {date, title, type}
  if (serverAvailable) {
    try {
  ...
  if (serverAvailable) {
    try {
      const res = await fetch(
        `${API_BASE}/racedays/${encodeURIComponent(dateStr)}`,
...
async function render() {
  ...
  for (const list of byDate.values()) {
    list.sort(
      (a, b) =>
        ROLE_ORDER.indexOf(a.role || "MC") -
          ROLE_ORDER.indexOf(b.role || "MC") || a.title.localeCompare(b.title)
    );
  }

  const todayStr = fmtDate(new Date());
  // async function render(){ ... の中にある forループを以下に置き換え
  ...
  for (const c of cells) {
    ...
    // ★変更点2: 日付表示に曜日を追加 (num作成部分をこれに統一)
    const num = document.createElement("div");
    num.className = "num";
    // スマホで縦並びになった時に曜日がないと困るため追加
    num.innerHTML = `${d.getDate()} <span class="wk-label">(${dayOfWeek})</span>`;
    ...
    const allEvents = (byDate.get(dateStr) || []).map((e) => ({
      ...e,
      role: e.role || "MC",
    }));

    // ★変更点3: 「表示する中身があるか」判定してクラスをつける
    // (開催情報がある OR 出演予定がある)
    const hasContent = rinfos.length > 0 || allEvents.length > 0;

    if (!hasContent) {
      cell.classList.add("empty-day"); // CSSで消すためのクラス
    } else {
...
async function refreshEventList(dateStr) {
  ...
  const list = (await fetchEventsByMonth(y, m)).filter(
    (e) => e.date === dateStr
  );
  if (list.length === 0) {
    const p = document.createElement("div");
    p.className = "listEmpty";
    p.textContent = "この日の予定はまだありません";
  ...
  for (const ev of list) {
    ...
    title.textContent = `[${ev.role || "MC"}] ${ev.title}`;
